...
public class CmdGraph implements IFunction {
    ...
    private double total(final Map<String, List<CustomEnchantment>> grouped,
                         final String key,
    ...
    void compute(final CustomEnchantment enchant, final CommandSender sender, final ItemStack item) {
        ...
        for (int i = 2; i <= maxModified; i++) {
            ...
            final Map<String, List<CustomEnchantment>> grouped = enchants.stream().collect(
                    Collectors.groupingBy(e -> e.getGroup().equals(CustomEnchantment.DEFAULT_GROUP) ? "d" + getNext()
                            : e.getGroup()));
    ...
    double compute(final List<Double> weights,
        ...
        for (int i = 0; i < weights.size(); i++) {
            if (!indices.contains(i)) {
                indices.add(i);
                total += weights.get(i) * compute(weights,
